...
class CarlaEnv(gym.Env):
    ...
    def load_scenario(self, scenario_name, seed=None):
        try:
            ...
            print(
                f"Scenario {scenario_name} not found! Loading random scenario {new_name}..."
            )
        ...
        if (
            self.__first_episode
            and self.__active_scenario_dict["map_name"]
            == self.__world.get_active_map_name()
        ):
    ...
    def __spawn_vehicle(self, s_dict):
        location = (
            s_dict["initial_position"]["x"],
            s_dict["initial_position"]["y"],
            s_dict["initial_position"]["z"],
        )
        rotation = (
            s_dict["initial_rotation"]["pitch"],
            s_dict["initial_rotation"]["yaw"],
            s_dict["initial_rotation"]["roll"],
        )
        try:
    ...
    def __toggle_lights(self):
        if (
            "night" in self.__world.get_active_weather().lower()
            or "noon" in self.__world.get_active_weather().lower()
        ):
    ...
    def __spawn_traffic(self, seed):
        if (
            not self.__random_traffic
            and self.__active_scenario_dict["traffic_density"] == "None"
        ):
            return
        ...
        self.__world.spawn_vehicles_around_ego(
            self.__vehicle.get_vehicle(),
            radius=100,
            num_vehicles_around_ego=num_vehicles,
            seed=seed,
        )
    ...
    def __get_situations(self, scenarios):
        ...
        if scenarios:
            self.situations_dict = {
                key: value
                for key, value in self.situations_dict.items()
                if value["situation"] in scenarios
            }

...
class Display:
    ...
    def __initialize_pygame_window(self, title):
        ...
        return pygame.display.set_mode(
            (configuration.IM_WIDTH, configuration.IM_HEIGHT)
        )
    ...
    def play_window(self):
        ...
        try:
            while True:
                ...
                self.__main_screen.fill(
                    (127, 127, 127)
                )  # Fill the main window with a gray background
                ...
                for idx, sensor in enumerate(self.__sensor_window_dict):
                    ...
                    x_position = configuration.MARGIN + col_idx * (
                        sub_surface_width + configuration.MARGIN
                    )
                    y_position = configuration.MARGIN + row_idx * (
                        sub_surface_height + configuration.MARGIN
                    )
                    ...
                    pygame.draw.rect(
                        ...
                        (50, 50, 50),
                        (
                            x_position - configuration.BORDER_WIDTH,
                            y_position - configuration.BORDER_WIDTH,
                            sub_surface_width + 2 * configuration.BORDER_WIDTH,
                            sub_surface_height + 2 * configuration.BORDER_WIDTH,
                        ),
                    ...
                    if (
                        sensor in self.__sensor_dict
                        and self.__sensor_dict[sensor].get_last_data() is not None
                    ):
                        pygame_surface = pygame.surfarray.make_surface(
                            self.__sensor_dict[sensor].get_last_data().swapaxes(0, 1)
                        )
                        self.__main_screen.blit(
                            pygame_surface, (x_position, y_position)
                        )

                    # Display sensor legend
                    font = pygame.font.Font(None, 24)
                    legend_text = font.render(
                        sensor.capitalize(), True, (255, 255, 255)
                    )
                    self.__main_screen.blit(
                        legend_text,
                        (x_position + 10, y_position + sub_surface_height - 30),
                    )

                # Display GNSS data
                if (
                    "gnss" in self.__sensor_dict
                    and self.__sensor_dict["gnss"].get_last_data() is not None
                ):
                ...
                # Display IMU data
                if (
                    "imu" in self.__sensor_dict
                    and self.__sensor_dict["imu"].get_last_data() is not None
                ):
                    imu_font = pygame.font.Font(None, 24)
                    imu_data = self.__sensor_dict["imu"].get_last_data()
                    imu_text = (
                        f"IMU Sensor: Acceleration {imu_data.accelerometer.x:.6f}, {imu_data.accelerometer.y:.6f}, {imu_data.accelerometer.z:.6f},"
                    ...
                    imu_text_rect.topleft = (
                        configuration.IM_WIDTH
                        - imu_text_rect.width
                        - configuration.MARGIN,
                        configuration.IM_HEIGHT - configuration.MARGIN,
                    )
                ...
                # Display vehicle speed vector
                if self.__vehicle.get_velocity() is not None:
                    velocity = self.__vehicle.get_velocity()
                    velocity_font = pygame.font.Font(None, 24)
                    velocity_text = f"Speed: {velocity.length()*3.6:.2f} km/h (X: {velocity.x*3.6:.2f} km/h, Y: {velocity.y*3.6:.2f} km/h, Z: {velocity.z*3.6:.2f} km/h)"
                    velocity_surface = velocity_font.render(
                        velocity_text, True, (255, 255, 255)
                    )
                    self.__main_screen.blit(
                        velocity_surface,
                        (
                            configuration.MARGIN,
                            configuration.IM_HEIGHT - 2 * configuration.MARGIN,
                        ),
                    )

                # Display the acceleration vector
                if self.__vehicle.get_acceleration() is not None:
                    acceleration = self.__vehicle.get_acceleration()
                    acceleration_font = pygame.font.Font(None, 24)
                    acceleration_text = f"Acceleration: {acceleration.length():.2f} m/s^2 (X: {acceleration.x:.2f}, Y: {acceleration.y:.2f}, Z: {acceleration.z:.2f})"
                    acceleration_surface = acceleration_font.render(
                        acceleration_text, True, (255, 255, 255)
                    )
                    self.__main_screen.blit(
                        acceleration_surface,
                        (
                            configuration.MARGIN,
                            configuration.IM_HEIGHT - 3 * configuration.MARGIN,
                        ),
                    )
    ...
    def play_window_tick(self):
        ...
        for idx, sensor in enumerate(self.__sensor_window_dict):
            ...
            self.__main_screen.blit(
                legend_text, (x_position + 10, y_position + sub_surface_height - 30)
            )
        ...
        if (
            ...
            self.__main_screen.blit(
                gnss_surface,
                (configuration.MARGIN, configuration.IM_HEIGHT - configuration.MARGIN),
            )
        ...
        if (
            ...
            imu_text_rect = imu_surface.get_rect()
            imu_text_rect.topleft = (
                configuration.IM_WIDTH - imu_text_rect.width - configuration.MARGIN,
                configuration.IM_HEIGHT - configuration.MARGIN,
            )
            self.__main_screen.blit(imu_surface, imu_text_rect)

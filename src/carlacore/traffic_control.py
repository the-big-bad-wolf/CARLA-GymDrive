...
class TrafficControl:
    ...
    def spawn_vehicles(self, num_vehicles=10, autopilot_on=False):
        ...
        for i in range(num_vehicles):
            ...
            while vehicle is None:
                ...
                try:
                    vehicle = self.__world.try_spawn_actor(
    ...
    def spawn_vehicles_around_ego(
        self, ego_vehicle, radius, num_vehicles_around_ego, seed=None
    ):
        if seed is not None:
            random.seed(seed)
    ...
    def spawn_pedestrians(self, num_walkers=10):
        ...
        walker_controller_bp = self.__world.get_blueprint_library().find(
        ...
        for _ in range(num_walkers):
            ...
            sidewalk_waypoint = self.__map.get_waypoint(
                spawn_location,
            ...
            try:
                walker = self.__world.spawn_actor(
                    walker_bp, carla.Transform(sidewalk_waypoint.transform.location)
                )
            except RuntimeError:
                continue
            self.__active_pedestrians.append(walker)

            walker_controller = self.__world.spawn_actor(
    ...
    def spawn_pedestrians_around_ego(
        self, vehicle_location, num_walkers=10, radius=25.0
    ):
        if num_walkers < 1:
            print("You need to spawn at least 1 pedestrian.")
            return
        ...
        for _ in range(num_walkers):
            ...
            while sidewalk_waypoint is None:
                random_offset = carla.Location(
                    x=random.uniform(-radius, radius), y=random.uniform(-radius, radius)
                )
                potential_location = vehicle_location + random_offset
                waypoint = self.__map.get_waypoint(
                    potential_location,
                    project_to_road=True,
                    lane_type=(carla.LaneType.Sidewalk),
                )
            ...
            walker_bp = random.choice(
                self.__world.get_blueprint_library().filter("walker.pedestrian.*")
            )
            try:
                walker = self.__world.spawn_actor(
                    walker_bp, sidewalk_waypoint.transform
                )
            except RuntimeError:
                continue
            self.__active_pedestrians.append(walker)
